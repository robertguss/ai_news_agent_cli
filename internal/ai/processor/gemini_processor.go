...
func (gp *GeminiProcessor) AnalyzeContentWithRetry(ctx context.Context, content string, cfg *config.Config) (*AnalysisResult, error) {
        ...
        err := retry.DoWithCallback(ctx, cfg.RetryConfig(), func() error {
                var e error
                result, e = gp.analyzeContentInternal(ctx, content)
                return e
        }, func(attempt int, err error) {
                logging.Retry("ai_analysis", attempt, err)
        })

        if err != nil {
                wrappedErr := errs.Wrap("analyze content with gemini", err)
                logging.Error("ai_analysis", wrappedErr)
